...
class MobileBottleneck(nn.Module):
    def __init__(self, inp, oup, kernel, stride, exp, se=False, nl='RE'):
        ...
        self.conv = nn.Sequential(
            # pw
            nn.Conv2d(inp + exp, exp, 1, 1, 0, bias=False),
            nn.BatchNorm2d(exp),
            self.nlin_layer,
            # dw
    ...
    def forward(self, x):
        if self.use_res_connect:
            return x + self.conv(torch.cat([x, self.dense_connect(x)], dim=1))
        else:
            return self.conv(torch.cat([x, self.dense_connect(x)], dim=1))
